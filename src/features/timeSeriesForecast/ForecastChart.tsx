...
function ForecastLineChart({
  ...
  return (
    <div className="forecast-recharts-frame" role="img" aria-label={`${variableLabel[targetVariable]}真实值${baselineEnabled ? "、物理基线" : ""}与 ORION 预测对比图`}>
  ...
  if (series.length === 0) {
    return (
      <div className="forecast-chart-card forecast-chart-card--empty" role="status">
        <p>暂无预测序列数据</p>
        <span>运行预测后将在此显示真实值{baselineEnabled ? "、物理基线" : ""}与 ORION 预测曲线。</span>
      </div>
    );
  }
  ...
  return (
    <div className="forecast-chart-card">
      <div className="forecast-chart-toolbar">
        ...
        <button className="forecast-expand-button" type="button" onClick={() => setIsExpanded(true)} aria-label="放大预测图">
          放大图表
        </button>
      </div>
      ...
      <div className="forecast-chart-readable-legend" aria-label="预测图坐标与图例">
        ...
        <span className="legend-panorama">ORION 预测</span>
      </div>
        ...
        <div className="forecast-chart-dialog-backdrop">
          <section className="forecast-chart-dialog" role="dialog" aria-label="放大预测图" aria-modal="true">
            <div className="forecast-chart-dialog-header">
              <div>
                <p>放大图表</p>
                <h2>{label}真实值{baselineEnabled ? "、物理基线" : ""}与 ORION 预测</h2>
              </div>
